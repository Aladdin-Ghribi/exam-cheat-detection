...
# Check what device the detector will use
print("\n" + "=" * 50)
...
try:
    ...
    if detector.device == 'cpu' and cuda_available:
        ...
        print(f"✅ Detector correctly using GPU with {detector.img_size}px images")
    else:
        print(f"ℹ️  Detector using CPU with {detector.img_size}px images (expected if no GPU)")
