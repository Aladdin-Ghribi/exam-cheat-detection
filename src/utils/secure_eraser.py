...
class SecureEraser:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=3) as response:
                ...
                if date_str:
                    ...
                    # Make sure it's offset-naive for comparison with simple datetime objects if needed, 
                    # but parsedate_to_datetime returns offset-aware (GMT).
                    # We'll convert to naive UTC or keep unaware based on usage. 
                    # Let's strip tzinfo to match datetime.now() nature usually being naive,
                    # OR better: use offset-aware for everything if possible.
                    # For simplicity with existing code using datetime.now(), we convert to naive local or UTC.
                    
                    # Converting to naive UTC for consistent math
                    return network_time.replace(tzinfo=None)
        except Exception as e:
            print(f"Time Check Warning: Could not reach trusted time server: {e}")
    ...
    def secure_delete_file(file_path, passes=1):
        ...
        try:
            ...
            # Attempt standard delete as fallback if secure delete fails
            try:
                if path.exists():
                    os.remove(path)
            except:
        ...
        for root, dirs, files in os.walk(path, topdown=False):
            ...
            for name in dirs:
                ...
                try:
                    ...
                    # If rmdir fails (e.g. system file hidden), try rmtree
