    ...
    def _smooth_landmarks(self, current_landmarks, track_id):
        ...
        for i, current in enumerate(current_landmarks):
            ...
            for j, hist_landmarks in enumerate(history):
                weight = (1.0 - self.smoothing_factor) * \
                    ((j + 1) / len(history))
                smoothed['x'] = smoothed['x'] * \
                    (1 - weight) + hist_landmarks[i]['x'] * weight
                smoothed['y'] = smoothed['y'] * \
                    (1 - weight) + hist_landmarks[i]['y'] * weight
                smoothed['z'] = smoothed['z'] * \
                    (1 - weight) + hist_landmarks[i]['z'] * weight
...
# class PoseDetector:
#     """
#     MediaPipe Pose detector for extracting 2D/3D coordinates of head, shoulders, and hands.
#     Enables gesture and posture analysis.
#     """
